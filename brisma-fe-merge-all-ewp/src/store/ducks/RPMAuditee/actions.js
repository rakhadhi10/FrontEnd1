...
export const fetchMCS = (project_id) => async (dispatch, getState, api) => {
  ...
  if (!error) {
    const mapData = {
      makers: data.makers_auditee || [],
      checkers: data.checkers_auditee || [],
      signers: data.signers_auditee || [],
    };
...
export const approveActionPlanMaker =
  (project_rpm_id, kkpt_id, rekomendasi_kkpt, action_plan_kkpt, lampiran) =>
  async (dispatch, getState, api) => {
    const { error } = await api.approveActionPlanMaker(
      getToken(getState()),
      project_rpm_id,
      kkpt_id,
      rekomendasi_kkpt,
...
export const approveAllAuditee =
  (project_rpm_id, kkpt_id, note) => async (dispatch, getState, api) => {
...
export const uploadFile = (options) => async (dispatch, getState, api) => {
  const { error, data } = await api.uploadFileRPM(
    getToken(getState()),
    options,
  );
  if (!error) {
    return { error: null, data: data.url[0] };
  } else {
    return { error };
  }
};
