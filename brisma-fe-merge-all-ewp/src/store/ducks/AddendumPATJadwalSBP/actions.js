...
export const submitForm =
  (pat_id, alasan_adendum) => async (dispatch, getState, api) => {
    ...
    const prepareStepOne = {
      name: formStepOne.kegiatan.nama,
...
export const submitEditForm =
  (pat_id, alasan_adendum) => async (dispatch, getState, api) => {
    ...
    // dispatch({ type: types.SUBMIT_SUCCESSFUL });
    // console.log("Final: ", result);
    // console.log("Total anggaran: ", total_anggaran);
    // return false;
...
export const fetchAllSbp = (pat_id) => async (dispatch, getState, api) => {
  ...
  filters.anggaran = filters.anggaran
    ...
    : "";
  const { error, data, page } = await api.getAllSbpAddendum(
    getToken(getState()),
    pat_id,
...
export const deleteJadwalSbp =
  (jadwal_sbp_id, pat_id) => async (dispatch, getState, api) => {
...
export const openEditModal =
  (pat_id, sbp_id) => async (dispatch, getState, api) => {
    const { error, data } = await api.getJadwalSbpByIdAddendum(
      getToken(getState()),
      pat_id,
      sbp_id,
    );
