...
export const fetchTargetAuditAIW =
  (pat_id) => async (dispatch, getState, api) => {
    dispatch({ type: types.FETCH_AIW_START });

    const { error, data } = await api.getTargetAuditAddendum(
      getToken(getState()),
      pat_id,
    );
    if (!error) {
      ...
      const totalExisting = Object.values(data.existing).reduce(
        (prev, curr) => (prev += Number(curr)),
        0,
      );
      const totalTarget = Object.values(data.target).reduce(
        (prev, curr) => (prev += Number(curr)),
        0,
      );
      ukerData.push({
        ...
        percent:
          totalExisting &&
          totalTarget &&
          Math.round((totalTarget / totalExisting) * 100),
      });
      ...
      const totalExistingEchannel = data.echannel.reduce(
        (prev, curr) => (prev += curr.existing),
        0,
      );
      const totalTargetEchannel = data.echannel.reduce(
        (prev, curr) => (prev += curr.target),
        0,
      );
      echannelData.push({
        name: "Total",
        ...
        percent:
          totalExistingEchannel &&
          totalTargetEchannel &&
          Math.round((totalTargetEchannel / totalExistingEchannel) * 100),
      });

      dispatch({
        type: types.FETCH_AIW_SUCCESSFUL,
        payload: { uker: ukerData, echannel: echannelData },
      });
    } else {
...
export const fetchTargetAuditAITI =
  (pat_id) => async (dispatch, getState, api) => {
    ...
    if (!error) {
      ...
      const totalExistingObjek = data.existing.reduce(
        (prev, curr) => (prev += Number(curr.count)),
        0,
      );
      const totalTargetObjek = data.target.reduce(
        (prev, curr) => (prev += Number(curr.count)),
        0,
      );
      objek.push({
        ...
        percent:
          totalExistingObjek &&
          totalTargetObjek &&
          Math.round((totalTargetObjek / totalExistingObjek) * 100),
      });
      ...
      dispatch({
        type: types.FETCH_AITI_SUCCESSFUL,
        payload: { objek, echannel },
      });
    } else {
...
export const fetchTargetAuditAIKP =
  (pat_id) => async (dispatch, getState, api) => {
    ...
    if (!error) {
      ...
      echannelData = data.echannel.map((e) => ({
        ...
        audit: e.target,
        percent:
          e.existing && e.target && Math.round((e.target / e.existing) * 100),
      }));
      const totalExistingEchannel = data.echannel.reduce(
        (prev, curr) => (prev += curr.existing),
        0,
      ...
      dispatch({
        type: types.FETCH_AIKP_SUCCESSFUL,
        payload: { uker: ukerData, echannel: echannelData },
      });
    } else {
