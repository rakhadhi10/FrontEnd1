...
export const submitForm =
  (pat_id, alasan_adendum) => async (dispatch, getState, api) => {
    ...
    const prepareStepOne = {
      ...
      orgeh_name: formStepOne.uker.orgeh && formStepOne.uker.orgeh.my_name,
      branch_induk: formStepOne.uker.branch && formStepOne.uker.branch.branch,
      branch_name: formStepOne.uker.branch && formStepOne.uker.branch.brdesc,
    ...
    if (selectedTipeAudit === "REG") {
      getRows(getState()).forEach((row) => {
        const a = {
          ...
          orgeh_name: row.orgeh.my_name,
          branch_name: row.branch.brdesc,
