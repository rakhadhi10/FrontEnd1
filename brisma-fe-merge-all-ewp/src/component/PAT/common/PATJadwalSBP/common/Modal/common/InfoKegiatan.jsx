...
import DebounceSBP from "../../../../../../AutoComplete/DebounceSBP";
    ...
    >
      <div className="mb-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          >
            <DebounceSBP placeholder="Ketik Nama Kegiatan" />
          </Form.Item>
          <Form.Item
            name="uker"
        ...
        >
          {
            ({ getFieldsValue, getFieldsError }) => {
              const { kegiatan, uker: { orgeh, branch }, start_date, end_date } = getFieldsValue(true)
              const anyErrors = getFieldsError().some(({ errors }) => errors.length)
              const allFilled = kegiatan && orgeh && branch && start_date && end_date && !anyErrors
              return (
